...
if [ "${1:-}" = "--verify" ]; then
  ...
  echo ""
  echo "  Start the services (each in its own terminal tab):"
  echo ""
  echo "    source .venv/bin/activate"
  echo "    python -m src.run_telegram    # Telegram bot (listens for your commands)"
  echo "    python -m src.run_trade       # trading loop (paper mode, runs every 60s)"
  echo "    python -m src.run_dashboard   # web dashboard → http://localhost:8080"
  echo ""
  echo "  What to expect:"
  echo "    • Within ~30s the bot posts a startup banner in your Telegram group."
  echo "    • Send it STATUS to get a live snapshot."
  echo "    • The dashboard starts empty and fills in as trades close."
  echo "    • Daily reports arrive at 8am (run python -m src.run_daily_report to test now)."
  echo ""
  echo "  Ready to run 24/7? See DEPLOYMENT.md for the full VPS setup."
  echo ""
  echo "  Tip: ask your AI agent to run the openbook-setup skill to be guided interactively."
  echo ""
fi

ok "Setup complete. Fill in .env, then run:  bash setup.sh --verify"
